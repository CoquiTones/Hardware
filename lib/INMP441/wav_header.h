...
typedef struct {
  ...
  uint32_t chunk_size;  /*!< This is the size of the rest of the chunk following
                           this number */
  char chunk_format[4]; /*!< Contains the letters "WAVE" */
} __attribute__((
    packed)) wav_descriptor_chunk_t; /*!< Canonical WAVE format starts with the
                                        RIFF header */

typedef struct {
  char subchunk_id[4];    /*!< Contains the letters "fmt " */
  uint32_t subchunk_size; /*!< PCM = 16, This is the size of the rest of the
...
__attribute__((packed)) pcm_wav_fmt_chunk_t; /*!< The "fmt " subchunk describes
                                                the sound data's format */

typedef struct {
  char subchunk_id[4];    /*!< Contains the letters "fmt " */
  uint32_t subchunk_size; /*!< ALAW/MULAW = 18, This is the size of the rest of
                             the Subchunk which follows this number */
  uint16_t audio_format;  /*!< ALAW = 6, MULAW = 7, values other than 1 indicate
  ...
  uint32_t byte_rate;       /*!< ALAW/MULAW = 8000, ==SampleRate * NumChannels *
                               BitsPerSample s/ 8 */
  uint16_t
      block_align; /*!< ALAW/MULAW = 1, ==NumChannels * BitsPerSample / 8 */
  uint16_t
      bits_per_sample; /*!< ALAW/MULAW = 8, 8 bits = 8, 16 bits = 16, etc. */
  uint16_t ext_size;   /*!< ALAW/MULAW = 0, Size of the extension (0 or 22) */
} __attribute__((
    packed)) non_pcm_wav_fmt_chunk_t; /*!< The "fmt " subchunk describes the
                                         sound data's format */
...
typedef struct {
  ...
  uint32_t subchunk_size; /*!< ==NumSamples * NumChannels * BitsPerSample / 8 */
    ...
    packed)) wav_data_chunk_t; /*!< The "data" subchunk contains the size of the
                                  data and the actual sound */

typedef struct {
  wav_descriptor_chunk_t descriptor_chunk; /*!< Canonical WAVE format starts
                                              with the RIFF header */
  pcm_wav_fmt_chunk_t
      fmt_chunk; /*!< The "fmt " subchunk describes the sound data's format */
  wav_data_chunk_t data_chunk; /*!< The "data" subchunk contains the size of the
                                  data and the actual sound */
 ...
 *
 */
#define PCM_WAV_HEADER_DEFAULT(wav_sample_size, wav_sample_bits,               \
                               wav_sample_rate, wav_channel_num)               \
  {                                                                            \
    .descriptor_chunk = {.chunk_id = {'R', 'I', 'F', 'F'},                     \
                         .chunk_size =                                         \
                             (wav_sample_size) + sizeof(pcm_wav_header_t) - 8, \
    ...
    .fmt_chunk = {.subchunk_id = {'f', 'm', 't', ' '},                         \
                  ...
                  .byte_rate = (wav_sample_bits) * (wav_sample_rate) *         \
                               (wav_channel_num) / 8,                          \
                  .block_align =                                               \
                      (uint16_t)((wav_sample_bits) * (wav_channel_num) / 8),   \
