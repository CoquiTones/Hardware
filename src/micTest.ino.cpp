...
void setup();
void loop();
#line 25 "/home/alo/Coquitones Hardware/Hardware/src/micTest.ino"
void setup() {
  Serial.begin(115200);
  while (!Serial) {
    delay(10);
  ...
  const char *result = microphone.recordFiveMinutesToFile(
      "/recording-with-sensors-no-cuts-maybee.wav");
